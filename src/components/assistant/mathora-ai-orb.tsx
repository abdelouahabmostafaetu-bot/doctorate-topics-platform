...
function renderLinks(text: string): React.ReactNode[] {
  ...
  const regex =
    /\[([^\]]+)\]\(((?:https?:\/\/|\/)[^\s)]+)\)|(https?:\/\/[^\s)\]<]+)|(?<![\w/])(\/(?:topics|search|universities|guide|coffee|revision|contribute|library|lectures|theses|mathora)[^\s)\]<]*)/g;
  let last = 0;
  let key = 0;
  ...
  while ((match = regex.exec(text))) {
    ...
    parts.push(
      <a
        ...
        className="font-semibold text-[#59677f] underline decoration-[#c7ceda] underline-offset-2 transition hover:text-[#2f3440] dark:text-[#b8c7e5] dark:hover:text-white"
      >
        {match[1] || "فتح الرابط"}
      </a>,
    );
...
export function MathoraAiOrb() {
  ...
  const loadStatus = useCallback(async () => {
    try {
      ...
      // Status is optional; POST reports real failures.
    }
  }, []);
        ...
        body: JSON.stringify({
          messages: next
            .map(({ role, content }, index, arr) => ({
              role,
              content:
                role === "user" && index === arr.length - 1 && topicSlug
                  ? `${content}\n\nالسياق الحالي: المستخدم في صفحة الموضوع /topics/${topicSlug}. إذا كان السؤال عن "هذا الموضوع" أو "هذه الصفحة" فاستعمل هذا الرابط وهذا الـ slug في البحث.`
                  : content,
            }))
            .slice(-10),
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-black/25 backdrop-blur-[2px] sm:bg-transparent" onClick={() => setOpen(false)}>
          <section
            ...
            className="fixed inset-x-3 bottom-3 top-14 flex flex-col overflow-hidden rounded-[1.7rem] border border-black/10 bg-[#fbfbfa] shadow-[0_24px_80px_rgba(15,15,15,0.25)] dark:border-white/10 dark:bg-[#191919] sm:inset-auto sm:bottom-24 sm:left-5 sm:h-[680px] sm:max-h-[calc(100vh-7rem)] sm:w-[440px]"
          >
            <div className="flex items-center gap-3 border-b border-[#e8e8e5] bg-white/85 px-4 py-3 backdrop-blur dark:border-[#2f2f2f] dark:bg-[#202020]/85">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <p className="text-sm font-bold tracking-tight text-[#37352f] dark:text-[#f4f4f5]">{BRAND} AI</p>
                  <span className="rounded-full bg-zinc-900 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-white dark:bg-white dark:text-zinc-900">Pro</span>
                </div>
                <p className="truncate text-[11px] text-[#787774] dark:text-[#9b9b9b]">
                  {topicSlug ? "يفهم الصفحة الحالية ويبحث في أرشيف المواضيع" : "مساعد بحث ومراجعة مبني على مواضيع DocMath DZ"}
                </p>
              </div>
              ...
              <button type="button" onClick={() => setOpen(false)} className="flex h-8 w-8 items-center justify-center rounded-full text-[#787774] transition hover:bg-[#f1f1ef] hover:text-[#37352f] dark:hover:bg-[#2a2a2a] dark:hover:text-white" aria-label="تصغير">
                ✕
              </button>
            </div>
            ...
            {signedIn === false ? (
              <div className="flex flex-1 flex-col items-center justify-center gap-3 px-6 text-center">
                <OrbLogo size={58} />
                <p className="font-semibold text-[#37352f] dark:text-[#f4f4f5]">Mathora AI للأعضاء فقط</p>
                <p className="max-w-xs text-sm leading-7 text-[#787774] dark:text-[#9b9b9b]">
                  سجّل الدخول لتستعمل المساعد في البحث داخل مواضيع الدكتوراه والحصول على اقتراحات مخصصة.
                </p>
                <Link href="/signin" className="rounded-full bg-zinc-900 px-5 py-2 text-sm font-semibold text-white transition hover:bg-zinc-700 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-200">
                  تسجيل الدخول
                </Link>
              </div>
            ) : (
              <>
                <div className="border-b border-[#e8e8e5] bg-[#f7f7f5]/70 px-4 py-2 dark:border-[#2f2f2f] dark:bg-[#202020]/45">
                  <div className="flex items-center gap-2 overflow-x-auto pb-1 [scrollbar-width:none]">
                    <button type="button" onClick={clearChat} className="shrink-0 rounded-full border border-[#e3e2e0] bg-white px-3 py-1 text-[11px] font-semibold text-[#787774] transition hover:bg-[#f1f1ef] dark:border-[#3a3a3a] dark:bg-[#202020] dark:hover:bg-[#2a2a2a]">
                      محادثة جديدة
                    </button>
                    {lastPrompt && !busy && (
                      <button type="button" onClick={() => send(lastPrompt)} className="shrink-0 rounded-full border border-[#e3e2e0] bg-white px-3 py-1 text-[11px] font-semibold text-[#787774] transition hover:bg-[#f1f1ef] dark:border-[#3a3a3a] dark:bg-[#202020] dark:hover:bg-[#2a2a2a]">
                        إعادة المحاولة
                      </button>
                    )}
                    {messages.length > 0 && (
                      <button type="button" onClick={() => copy(transcript(), "transcript")} className="shrink-0 rounded-full border border-[#e3e2e0] bg-white px-3 py-1 text-[11px] font-semibold text-[#787774] transition hover:bg-[#f1f1ef] dark:border-[#3a3a3a] dark:bg-[#202020] dark:hover:bg-[#2a2a2a]">
                        {copied === "transcript" ? "تم النسخ" : "نسخ المحادثة"}
                      </button>
                    )}
                    {topicSlug && (
                      <button type="button" onClick={() => send("اشرح لي هذا الموضوع واقترح مواضيع مشابهة له")} className="shrink-0 rounded-full border border-[#e3e2e0] bg-white px-3 py-1 text-[11px] font-semibold text-[#787774] transition hover:bg-[#f1f1ef] dark:border-[#3a3a3a] dark:bg-[#202020] dark:hover:bg-[#2a2a2a]">
                        افهم هذه الصفحة
                      </button>
                    )}
                  </div>
                ...
                <div ref={listRef} className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4">
                  {!hasChat && (
                    <div className="pt-5 text-center">
                      ...
                      <p className="mx-auto mt-2 max-w-sm text-[13px] leading-7 text-[#787774] dark:text-[#9b9b9b]">
                        اسألني عن جامعة، سنة، تخصص، تمرين، أو خطة مراجعة. سأعطيك إجابة مركزة وروابط مباشرة من DocMath DZ.
                      </p>
                      <div className="mt-4 grid gap-2 text-right sm:grid-cols-2">
                        {ACTIONS.map((a) => (
                          <button key={a.title} type="button" onClick={() => send(a.prompt)} className="rounded-2xl border border-[#e3e2e0] bg-white px-3.5 py-2.5 text-[12.5px] text-[#37352f] shadow-sm transition hover:-translate-y-0.5 hover:border-[#c7c7cc] hover:bg-[#f7f7f5] dark:border-[#3a3a3a] dark:bg-[#202020] dark:text-[#ececec] dark:hover:border-[#555]">
                            <span className="block font-bold">{a.title}</span>
                            <span className="mt-0.5 block text-[11px] text-[#787774] dark:text-[#9b9b9b]">{a.prompt}</span>
                          </button>
                        ))}
                      </div>
                    </div>
                  ...
                  {messages.map((m) =>
                    m.role === "user" ? (
                      <div key={m.id} className="flex justify-start">
                        <div dir="auto" className="max-w-[86%] whitespace-pre-wrap rounded-2xl rounded-tr-sm bg-[#37352f] px-3.5 py-2 text-[13px] leading-6 text-white dark:bg-[#e8e8e8] dark:text-[#191919]">
                          {m.content}
                        </div>
                      </div>
                    ) : (
                      <div key={m.id} className="group flex gap-2.5 text-right">
                        <OrbLogo size={24} />
                        <div className="min-w-0 flex-1">
                          <div className="mb-1 flex items-center gap-2">
                            <p className="text-[11px] font-semibold text-[#9b9a97] dark:text-[#787878]">{BRAND}</p>
                            <div className="flex opacity-0 transition group-hover:opacity-100">
                              <button type="button" onClick={() => copy(m.content, m.id)} className="rounded px-1 text-[10px] text-[#9b9a97] hover:bg-[#f1f1ef] dark:hover:bg-[#2a2a2a]">
                                {copied === m.id ? "تم النسخ" : "نسخ"}
                              </button>
                              <button type="button" onClick={() => rate(m, "up")} className={`rounded px-1 text-[10px] hover:bg-[#f1f1ef] dark:hover:bg-[#2a2a2a] ${m.feedback === "up" ? "text-emerald-600" : "text-[#9b9a97]"}`}>
                                👍
                              </button>
                              <button type="button" onClick={() => rate(m, "down")} className={`rounded px-1 text-[10px] hover:bg-[#f1f1ef] dark:hover:bg-[#2a2a2a] ${m.feedback === "down" ? "text-red-600" : "text-[#9b9a97]"}`}>
                                👎
                              </button>
                            </div>
                          </div>
                          <div dir="auto" className="whitespace-pre-wrap text-[13px] leading-7 text-[#37352f] dark:text-[#e8e8e8]">
                            {renderLinks(m.content)}
                          </div>
                        </div>
                      </div>
                    ),
                  ...
                  {stream && (
                    <div className="flex gap-2.5 text-right">
                      <OrbLogo size={24} active />
                      <div className="min-w-0 flex-1">
                        <p className="mb-1 text-[11px] font-semibold text-[#9b9a97] dark:text-[#787878]">{BRAND}</p>
                        <div dir="auto" className="whitespace-pre-wrap text-[13px] leading-7 text-[#37352f] dark:text-[#e8e8e8]">
                          {renderLinks(stream)}
                  ...
                  {busy && !stream && (
                    <div className="flex items-center gap-2 text-[13px] text-[#9b9a97]">
                      <OrbLogo size={24} active />
                      <span className="animate-pulse">يبحث في أرشيف المواضيع…</span>
                    </div>
                  )}
                ...
                <div className="border-t border-[#e8e8e5] bg-white/85 p-3 backdrop-blur dark:border-[#2f2f2f] dark:bg-[#202020]/85">
                  <div className="rounded-2xl border border-[#deded9] bg-white shadow-sm focus-within:border-[#b9b9b1] dark:border-[#3a3a3a] dark:bg-[#191919] dark:focus-within:border-[#5a5a5a]">
                    <textarea
                      ...
                      onChange={(event) => setInputSmart(event.target.value)}
                      onKeyDown={(event) => {
                      ...
                      dir="auto"
                      placeholder={exhausted ? "عد لاحقًا بعد تجدد الرصيد" : "اكتب رسالتك إلى Mathora AI..."}
                    ...
                    <div className="flex items-center gap-2 px-2.5 pb-2.5">
                      <Link href="/mathora" className="rounded-full px-2 py-1 text-[11px] font-medium text-[#787774] transition hover:bg-[#f1f1ef] dark:text-[#9b9b9b] dark:hover:bg-[#2a2a2a]">
                        فتح كامل
                      </Link>
                      <button type="button" onClick={toggleVoice} disabled={busy || exhausted} className={`rounded-full px-2 py-1 text-[11px] font-medium transition hover:bg-[#f1f1ef] disabled:opacity-40 dark:hover:bg-[#2a2a2a] ${listening ? "text-red-600" : "text-[#787774] dark:text-[#9b9b9b]"}`}>
                        {listening ? "إيقاف الصوت" : "صوت"}
                      </button>
                      <span className="flex-1 text-[10.5px] text-[#9b9a97] dark:text-[#787878]">
                        {busy ? "يتم التوليد الآن" : listening ? "أستمع الآن…" : input.trim() ? `${input.trim().length}/${MAX_INPUT}` : "Enter للإرسال · Ctrl+K للفتح"}
                      </span>
                      {busy ? (
                        <button type="button" onClick={stopGenerating} className="rounded-full border border-[#e3e2e0] px-4 py-1.5 text-[12px] font-bold text-[#37352f] transition hover:bg-[#f1f1ef] dark:border-[#3a3a3a] dark:text-[#f4f4f5] dark:hover:bg-[#2a2a2a]">
                          إيقاف
                        </button>
                      ) : (
                        <button type="button" onClick={() => send()} disabled={exhausted || !input.trim()} className="rounded-full bg-[#37352f] px-4 py-1.5 text-[12px] font-bold text-white transition hover:bg-[#111] disabled:opacity-40 dark:bg-white dark:text-[#191919] dark:hover:bg-zinc-200">
                          إرسال
                        </button>
                      )}
                    </div>
