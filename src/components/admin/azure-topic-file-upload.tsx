...
export function AzureTopicFileUpload({ topicId, kind, hasFile }: { topicId: string; kind: Kind; hasFile: boolean }) {
  ...
  async function start() {
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        xhr.onload = () => xhr.status >= 200 && xhr.status < 300 ? resolve() : reject(new Error(`رفض Azure الرفع (${xhr.status}). تحقق من CORS.`));
      ...
      setProgress(100); setMessage("تم الحفظ في Azure بنجاح."); router.refresh();
  ...
  return <div className="mt-3 space-y-2">
    <div className="flex flex-wrap items-center gap-2">
      <input ref={input} type="file" accept="application/pdf,.pdf" disabled={busy} className="max-w-[190px] text-xs" />
      <button type="button" onClick={start} disabled={busy} className="rounded-md border px-3 py-1.5 text-xs transition hover:border-primary hover:text-primary disabled:opacity-50">{busy ? `${progress}%` : hasFile ? "استبدال في Azure" : "رفع كامل إلى Azure"}</button>
    </div>
    {busy && <div className="h-1.5 overflow-hidden rounded-full bg-muted"><div className="h-full bg-primary transition-all" style={{ width: `${progress}%` }} /></div>}
