...
export default async function AdminLecturesPage() {
	const [universities, specialties, modules] = await Promise.all([
		...
		prisma.module.findMany({ include: { university: true, specialty: true, resources: { orderBy: { createdAt: "desc" } } }, orderBy: [{ level: "asc" }, { semester: "asc" }, { name: "asc" }] }),
	]);
	...
	const moduleOptions = modules.map((m) => ({ id: m.id, label: `${m.university.nameAr?.trim() || m.university.name} · ${levelByValue(m.level)?.label ?? m.level} · س${m.semester}${m.specialty ? ` · ${m.specialty.nameAr?.trim() || m.specialty.name}` : ""} · ${m.name}` }));
			...
			<div className="grid gap-5 lg:grid-cols-2">
				<section className="overflow-hidden rounded-2xl border bg-card shadow-sm">
					...
					<form action={createModule} className="grid gap-4 p-4 sm:grid-cols-2">
						...
						<label className="text-xs font-medium">التخصص (للماستر)<select name="specialtyId" className={fieldClass}><option value="">بدون تخصص</option>{specialties.map((s) => <option key={s.id} value={s.id}>{s.nameAr?.trim() || s.name}</option>)}</select></label>
