...
async function requireSuperAdmin() {
  ...
  if (!user || user.role !== "SUPER_ADMIN") {
    throw new Error("This feature is for the super admin only");
  }
  return user;
}
...
function assertNoteCol(col: string) {
  if (!isAllowedNoteCollection(col)) throw new Error("Note collection not allowed");
}

function assertNotebookCol(col: string) {
  if (!isAllowedNotebookCollection(col)) throw new Error("Notebook collection not allowed");
}
...
export async function createNoteAction(input: {
  ...
  const now = new Date();
  const doc = {
    title: clean(input.title, 300) || "New note",
...
export async function updateNoteAction(input: {
  ...
  if (input.title !== undefined) $set.title = clean(input.title, 300) || "(untitled)";
